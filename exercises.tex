...
\begin{rmk}
In the finite-dimensional case, how do we actually know that there is no natural way to choose an isomorphism \(V\iso V^*\)? What would it even mean for there to be a natural way? Arguably, for such an isomorphism to be natural it should at least respect change of basis in~\(V\).

Let \(\V\)~be the subcategory of~\(\Vect\) consisting of the finite-dimensional spaces (over some fixed field of characteristic zero) and the automorphisms of these spaces. Let \(\inv{(-)}:\Vop\to\V\) denote the inversion functor and \((-)^*:\Vop\to\V\) the dual functor. An isomorphism \(V\iso V^*\) respecting change of basis would be a natural isomorphism \(\zeta:\inv{(-)}\iso(-)^*\)---for any automorphism \(f:V\to V\), the following diagram would commute:
\begin{diagram}
V				&\rTo^{\zeta_V}_{\iso}	&V^*\\
\dTo<{\inv{f}}	&						&\dTo>{f^*}\\
V				&\rTo_{\zeta_V}^{\iso}	&V^*
\end{diagram}
In bracket notation, this is equivalent to
\[\sprod{x}{\zeta_V(y)}=\sprod{f(x)}{\zeta_V(f(y))}\tag{1}\]
for all \(x,y\in V\). However, it can be proved that there is no such~\(\zeta\) satisfying~(1) for all~\(V\) in~\(\V\).\footnote{See for example Exercise~9.8 in~\cite{blyth}.} Therefore there is no natural isomorphism \(V\iso V^*\).
\end{rmk}
...
\bibitem{blyth} Blyth, T. \textit{Module Theory: An Approach to Linear Algebra.} Oxford, 1977.
