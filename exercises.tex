...
\section*{Chapter~4}
\begin{rmk}
The homomorphism theorem for groups (Theorem~4.10) shows that for a group homomorphism \(h:G\to H\), \(\ker h\)~is universal among the normal subgroups of~\(G\) factorization through which recovers~\(h\). Equivalently, \(G/\ker h\)~is universal among quotients through which \(h\)~is recovered.
...
Intuitively, as \(N\)~ranges from \(1\)~to~\(K\), \(G/N\)~``collapses'' more and more of the structure of~\(G\) while still allowing recovery of~\(h\). Since \(G/K\)~is ``smallest'' with this property, it is always possible to collapse from~\(G/N\) to \(G/K\) and still recover~\(h\).
