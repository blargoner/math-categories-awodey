...
Now define \(P:\Hom_{\C}(A,B)\to\Hom_{\C}(1\times A,B)\) by \(P(f)=f\after p:1\times A\to B\) for \(f:A\to B\), and define \(Q:\Hom_{\C}(1\times A,B)\to\Hom_{\C}(A,B)\) by \(Q(g)=g\after q:A\to B\) for \(g:1\times A\to B\). Then
\[(Q\after P)(f)=Q(P(f))=Q(f\after p)=(f\after p)\after q=f\after(p\after q)=f\after1_A=f\]
and
\[(P\after Q)(g)=P(Q(g))=P(g\after q)=(g\after q)\after p=g\after(q\after p)=g\after1_{1\times A}=g\]
Therefore \(Q\after P=1_{\Hom_{\C}(A,B)}\) and \(P\after Q=1_{\Hom_{\C}(1\times A,B)}\), so \(P\)~and~\(Q\) are mutually inverse and hence bijective.
\end{proof}

\begin{exer}[12]
Let \(\C\)~be a cartesian closed category and \(C\in\C\). Exponentiation with base object~\(C\) gives a contravariant functor \(C^{(-)}:\dual{\C}\to\C\).
\end{exer}
\begin{proof}
...
\end{diagram}
Then \(C^{(-)}\)~maps objects to objects and arrows to arrows and preserves domains and codomains of arrows in~\(\dual{\C}\). Also
...
It follows that \(C^{g\after f}=C^f\after C^g\) and hence \(C^{(-)}\)~preserves composites in~\(\dual{\C}\). This completes the proof that \(C^{(-)}\)~is a contravariant functor.
\end{proof}
...
More specifically, \(F\)~is a functor if and only if for each fixed \(A\in\A\) and \(B\in\B\), \(F(A,-)\)~and~\(F(-,B)\) are functors and for any \(\alpha:A\to A'\) and \(\beta:B\to B'\), \(F(A,\beta)\)~is a natural transformation from \(F(-,B)\)~to~\(F(-,B')\) and \(F(\alpha,B)\)~is a natural transformation from \(F(A,-)\)~to~\(F(A',-)\):\footnote{Technically speaking, \(F(A,\beta)\)~is the \emph{component} of the natural transformation~\(F(-,\beta)\) at~\(A\), and similarly for \(F(\alpha,B)\).}
\begin{diagram}
...
\[\Hom:\dual{\C}\times\C\to\Sets\]
inducing the familiar representable functors
\[\Hom(C,-):\C\to\Sets\qquad\Hom(-,C):\dual{\C}\to\Sets\]
\end{exer}
\begin{proof}
...
Let \(I\)~be a set. Then
\[\Sets^I\eqv\Sets/I\]
and this equivalence is ``natural'' in the sense that for any function \(f:J\to I\), the following diagram commutes, where \(\Sets^f\) is the reindexing functor, and \(\pull{f}\)~is the pullback functor:
\begin{diagram}
\Sets^I			&\rTo	&\Sets/I\\
...
\[\Phi_I((A_i)_{i\in I})=[p_i]:\coprod_{i\in I}A_i\to I\]
...
where \(\mu_i:B_i\to\coprod_{i\in I}B_i\) the \(i\)-th coproduct injection.
\end{itemize}
...
\end{align*}
It is immediate that \(\Sets^f\)~is a functor (the ``reindexing functor''). We already know that pullback \(\pull{f}:\Sets/I\to\Sets/J\) is a functor (Proposition~5.10). We claim that \(\Phi_J\after\Sets^f=\pull{f}\after\Phi_I\), which is equivalent to \(\Sets^f=\Psi_J\after\pull{f}\after\Psi_I\). This follows from the pullback diagram
\begin{diagram}
\coprod_{j\in J}A_{f(j)}&\rTo&\coprod_{i\in I}A_i\\
\dTo<{\pi_J}&&\dTo>{\pi_I}\\
J&\rTo_f&I
\end{diagram}
where \(\pi_J\)~and~\(\pi_I\) are indexing projections and the upper arrow is the reindexing function \((x,j)\mapsto(x,f(j))\).
\end{proof}
\begin{rmk}
