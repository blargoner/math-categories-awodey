...
\end{diagram}
Taking \(\eta=\theta\) in~(2) shows that \(\vartheta\after x=\vartheta_{(x,C)}\). If \(\varphi:P\to Q\) satisfies \(\varphi\after x=\vartheta_{(x,C)}\), then taking \(\eta=\varphi\) in~(2) shows that \(\varphi=\vartheta\).
\end{rmk}
