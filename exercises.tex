...
In particular, the equivalence \(\Par\eqv\Setsp\) is an adjoint equivalence.\footnote{This phenomenon actually occurs for \emph{any} adjoint equivalence.}
\end{rmk}
\begin{proof}
...
b&\text{otherwise}
\end{cases}\]
This means that the identity natural transformation \(1_{1_{\Par}}:1_{\Par}\to 1_{\Par}=GF\) is the unit of the adjunction \(F\adj G\).

On the other hand, given a pointed set~\((A,a)\), set~\(B\), and partial function \(f:A-\{a\}\parto B\), there is a pointed function \(g:(A,a)\to B_*\) unique with \(Gg=f\) given by
\[g(x)=\begin{cases}
f(x)&\text{if }x\in U_f\\
{*}&\text{otherwise}
\end{cases}\]
So \(1_{1_{\Par}}\)~is also the counit of the adjunction \(G\adj F\).
\end{proof}
...
\[\powBAop\adj\ult\]
In particular, the equivalence \(\BAf\eqv\Setsfop\) is an adjoint equivalence.
\end{rmk}
\begin{proof}
