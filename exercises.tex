...
\end{diagram}
Multiplying on the left by~\(\psi_C\) and on the right by~\(\psi_B\), we obtain
\[\psi_C\after G\alpha=F\alpha\after\psi_B\]
as desired. It is immediate that \(\psi\after\vartheta=1_F\) and \(\vartheta\after\psi=1_G\), so \(\vartheta\)~is an iso.
\end{proof}
\begin{rmk}
