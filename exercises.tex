...
\section*{Chapter~6}
\begin{rmk}
Let \(\C\)~be a category with binary products and let \(B,C\in\C\). An object \(C^B\in\C\) is an exponential for \(B\) and~\(C\) if and only if there is an isomorphism
\[\Hom_{\C}(A\times B,C)\iso\Hom_{\C}(A,C^B)\tag{1}\]
for all \(A\in\C\) which is natural in~\(A\).\footnote{The concept of naturality is defined in Chapter~7.}
\end{rmk}
\begin{proof}
