...
\begin{rmk}
We can also consider the concept of a group (or monoid) in a \emph{monoidal} category, which uses the monoidal product~\(\mprod\) instead of~\(\times\) in its definition. For example, a monad on a category~\(\C\) is a monoid in the monoidal category~\(\C^{\C}\) of endofunctors of~\(\C\) under composition.\footnote{See Chapters 7 and~10.}
\end{rmk}
