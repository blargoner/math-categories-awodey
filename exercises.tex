...
\begin{exer}[5]
For graphs \(G\) and~\(H\), define \(H^G\) to be the graph whose vertices are arbitrary functions \(\varphi:G_v\to H_v\) and whose edges \(\theta:\varphi\to\psi\) are functions \(\theta:G_e\to H_e\) making these diagram commute:
